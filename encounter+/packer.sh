...
TMP_DIR="$BASE_DIR/.tmp_pack"
mkdir -p "$TMP_DIR"
PACK_FILE="$TMP_DIR/pack.xml"
...
function process_dir () {
  ...
  echo "process_dir $1"
  local dir="$1"
  local parent_base=$([ -n "$2" ] && echo -e "${2}_" || echo "")
  ...
  for file in "$dir"/*; do
    ...
    if [ -d "$file" ]; then
      ...
      local resource_bname=`basename "$file"`
        ...
        cp "$file" "$TMP_DIR/$parent_base${resource_bname}"
    	fi
    fi
  done
  popd
...
pushd "$TMP_DIR"
ls -l
zip -r "${PACK}.zip" * -x "${PACK}.zip"
popd
mv "$TMP_DIR/${PACK}.zip" "../${PACK}.pack"
rm -rf "$TMP_DIR"
popd
